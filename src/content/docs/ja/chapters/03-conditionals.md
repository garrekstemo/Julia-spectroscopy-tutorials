...
2. `==` と `===` の違いは何でしょうか?
3. `"Hello world"[i:j] == "o wo"` が `true` を返すように、`i` と `j` を選んでみましょう。
...
## 短絡評価
論理演算子 `&&` と `||` は *短絡評価 (short-circuit evaluation)* を行います。結果を決めるために必要な分だけ評価し、不要な式は評価しません。これはコンパクトな `if` の代わりとしてよく使われます。
